...
const UpdatePage = () => {
  ...
  const postId = searchParams.get("id"); // Assume page ID is passed in URL query
  ...
  useEffect(() => {
    if (!postId) {
      setErrors({ form: "No page ID provided" });
      setIsLoading(false);
      return;
    }
    ...
    const fetchPage = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          setErrors({ form: "Failed to load page data" });
        }
      } catch (error) {
        setErrors({ form: "An error occurred while fetching page data" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.success) {
        setErrors({ form: response.message || "Failed to update page" });
      } else {
